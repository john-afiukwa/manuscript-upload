...
export default function SideBar() {
  ...
  return (
    <div className="relative max-h-screen sticky top-0">
      <button className="mt-10 ml-4 p-2 md:hidden">
        <FaBarsStaggered className="text-lg" onClick={toggleMenuBar} />
      </button>

      <section
        className={`absolute md:block h-screen top-0 ${
          open ? "w-72" : "w-[80px]"
        } duration-300 md:relative absolute ${
          openMenu ? "left-0" : "left-[-20rem] md:left-0"
        }`}
      >
        <BsArrowLeftShort
